...
export const insertChild = async (data: any, op_number: string) => {
  ...
  const query = `
    INSERT INTO child (hn_number, firstname, lastname, age_text, dob, sex, nationality, active_status) 
    VALUES ($1, $2, $3, $4, $5, $6, $7, '1')
  `;
  try {
    await getClient().query(query, [data.hn, data.firstname, data.lastname, data.age_text, data.dob, data.sex, data.nationality]);
...
export const insertParent = async (data: any, op_number: string) => {
  const query = `
    INSERT INTO parent (hn_number, firstname, lastname, age_text, dob, sex, nationality, active_status) 
    VALUES ($1, $2, $3, $4, $5, $6, $7, '1')
  `;
  try {
    await getClient().query(query, [data.hn, data.firstname, data.lastname, data.age_text, data.dob, data.sex, data.nationality]);
...
export const updateChild = async (hn: string, data: { firstname: string, lastname: string, age_text: string, nationality: string, dob: string, sex: string }, op_number: string) => {
  const query = `
    UPDATE child 
...
export const updateParent = async (hn: string, data: { firstname: string, lastname: string, age_text: string, nationality: string, dob: string, sex: string }, op_number: string) => {
  const query = `
    UPDATE parent 
    SET firstname = $2, lastname = $3, age_text = $4, dob = $5, sex = $6, nationality = $7
    WHERE hn_number = $1 AND active_status = '1'
